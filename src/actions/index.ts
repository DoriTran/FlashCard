...
export async function getTwoLastestPack(): Promise<Pack[]> {
  const packIds = store.get("packIds")?.slice(0, 2) || [];
  const lastestPacks = packIds.map((packId: string) => store.get(packId));
  return lastestPacks;
}
